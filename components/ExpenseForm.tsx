...
import { Check, Calendar, Tag, FileText, Repeat, CalendarDays, ArrowRight, Lock, CheckCircle } from 'lucide-react';
...
export const ExpenseForm: React.FC<Props> = ({ 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isRestrictedBudgetMode) return;

    const val = parseFloat(amount);
    if (isNaN(val) || val <= 0) return;

    // Use preserved creator if editing, otherwise current user
    const createdBy = initialData ? initialData.createdBy : currentUser.name;
    ...
    if (isEditing && onUpdate) {
        ...
        setSuccessMessage('Saved successfully! You can add another.');
    }

    if (onClose) {
        onClose();
    } else {
        // Reset form for next entry but keep some reasonable defaults
        setAmount('');
        setDescription('');
        // We don't reset category or date as user might be entering multiple receipts from same event
    }
  };

  return (
    <Card className="max-w-2xl mx-auto shadow-lg border-indigo-100 relative" title={isEditing ? "Edit Transaction" : (type === TransactionType.PLANNED ? "Plan Budget" : "Record Expense")}>
      
      {successMessage && (
          <div className="mb-4 bg-emerald-50 text-emerald-700 p-3 rounded-lg border border-emerald-100 flex items-center gap-2 animate-fade-in">
              <CheckCircle size={18} />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg w-full md:w-fit">
            ...
            >
                <span className="font-sans">¥</span> Actual Expense
            </button>
            <button
                type="button"
                disabled={isEditing}
        ...
        {isRestrictedBudgetMode ? (
            ...
            <>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        <select 
                                            ...
                                            className="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-lg text-slate-800 font-medium focus:ring-2 focus:ring-indigo-500 outline-none appearance-none cursor-pointer"
                                        >
                                            {YEARS.map(y => (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-700 flex items-center gap-2">
                            <span className="font-sans font-bold">¥</span> Amount {type === TransactionType.PLANNED && !isEditing && <span className="text-slate-400 font-normal">(Per Month)</span>}
                        </label>
                        <div className="relative">
                ...
                <div className="flex items-center justify-between text-xs text-slate-400 px-1">
                    <span>Creator: <span className="font-medium text-slate-600">{initialData?.createdBy || currentUser.name}</span></span>
                </div>
